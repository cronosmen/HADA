...
//Rare exceptions to indicate a bug in the _directory code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_directory = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_directory]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.directory.module"
